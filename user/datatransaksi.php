...
<body>
	...
	<div style=" margin-bottom: 100px;" class="container">
		<h2 class="alert alert-warning text-center mt-5">DATA PESANAN</h2>
		...
		<div class="card">
		    ...
		    <h2 style="font-weight:bold; text-transform: uppercase;"><?php
		     ...
		     ?></h2>
		  	...
		  	<div class="card-body">
		  		<div style="margin-left:10px;" class="row mt-2">

		  			<?php
		  			$no = 1;
		  			$n=$_SESSION['username'];
					$nama= mysqli_query($koneksi,"SELECT nama FROM tamu WHERE username = '$n'");
					$m= mysqli_fetch_array($nama);
					$k= $m['nama'];
				  	$data = mysqli_query($koneksi,"SELECT * FROM pemesanan INNER JOIN kamar on pemesanan.tipe = kamar.tipe WHERE pemesanan.nama = '$k'");
				  	...
				  	while ($d = mysqli_fetch_array($data)) {
						...
						$jum = $ha * $d['harga'] * $d['jumlah'];
				  		...
				  		<div class="card mb-2" style="width: 18rem; margin-left:5px;">
						  <ul style="margin:10px;" class="list-group list-group-flush">
						  	<li class="list-group-item text-center">NO.<?php echo $no++; ?></li>
						    ...
						    <li class="list-group-item">jumlah 	: <?php echo $d['jumlah']; ?> kamar</li>
						    ...
						    <li class="list-group-item text-center"><form method="POST" action="bayar.php">
						<input name="idbayar" value="<?= $d['idpesan'] ?>" hidden>
