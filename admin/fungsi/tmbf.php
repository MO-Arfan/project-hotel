<?php
	...
	$data = mysqli_query($koneksi,"INSERT INTO fasilitas VALUES ('','$nama','$des','$gambar')");
	$query = mysqli_query($koneksi,"SELECT * FROM fasilitas");			

	$cek = mysqli_num_rows($query);

	if ($cek > 0) {
		echo "<script>alert('Data berhasil ditambah!');window.location='../daftarfasilitas.php'</script>";
    	exit;
	}else{
		echo "<script>alert('Data gagal ditambah!');window.location='../daftarfasilitas.php'</script>";
    	exit;
	}
}
