...
include 'fungsi/koneksi.php';
	...
	$query = "UPDATE pemesanan SET status = 'check in' where idpesan = '$id'";
	...
	if ($cek > 0) {
		...
		if ($h > 0) {
			if (mysqli_query($koneksi,$query)) {
				...
				echo "<script>alert('Check in Berhasil');window.location='javascript:history.back()';</script>";
				exit;
			}else{
				echo "<script>alert('Gagal Check in');window.location='javascript:history.back()';</script>";
				exit;
			}
		}else{
			echo "<script>alert('Stok habis);window.location='javascript:history.back()';</script>";
			exit;
		}
	}else{
